...
describe('validateWord', () => {
  const mockDictionary = new Set(['TEST', 'RATE', 'TARTE', 'ETAT', 'TATE', 'RARE', 'TARTINE', 'AEIOU', 'YETI']);
  ...
  it('should reject empty word', () => {
    expect(validateWord('', baseConstraints, dictionaryCheck).isValid).toBe(false);
  });

  it('should reject word not in dictionary', () => {
    expect(validateWord('XYZ', baseConstraints, dictionaryCheck).error).toBe('Mot inconnu');
  });
  ...
  it('should validate imposed letter', () => {
    ...
    expect(validateWord('RARE', baseConstraints, dictionaryCheck).error).toBe('Doit contenir T');
  });

  it('should validate forbidden letter', () => {
    // "TATE" has T (imposed ok), but has A (forbidden).
    expect(validateWord('TATE', baseConstraints, dictionaryCheck).error).toBe('Ne doit pas contenir A');
  });

  it('should validate correct word (free card)', () => {
    // We need a word with T, no A.
    // "YETI" -> has T, no A. In dict.
    expect(validateWord('YETI', baseConstraints, dictionaryCheck).isValid).toBe(true);
  });
  ...
  describe('Constraint Cards', () => {
    it('min_len', () => {
      ...
      expect(validateWord('YETI', constraints, dictionaryCheck).error).toBe('Minimum 5 lettres');
    });
    ...
    it('starts_with_imposed', () => {
      ...
      expect(validateWord('TARTE', constraints, dictionaryCheck).isValid).toBe(true);
      // ETAT has T, but starts with E. Fail.
      expect(validateWord('ETAT', constraints, dictionaryCheck).error).toBe('Doit commencer par T');
    });
    ...
    it('ends_with_imposed', () => {
      ...
      expect(validateWord('ETAT', constraints, dictionaryCheck).error).toBe('Doit finir par E');
    });
    ...
    it('unique_chars', () => {
      ...
      expect(validateWord('RATE', constraints, dictionaryCheck).isValid).toBe(true);
      // RARE -> R, A, R, E (R repeated). Fail.
      expect(validateWord('RARE', constraints, dictionaryCheck).error).toBe('Lettres doivent être uniques');
    });
    ...
    it('min_vowels', () => {
      ...
      // AEIOU -> 5 vowels. OK.
      expect(validateWord('AEIOU', constraints, dictionaryCheck).isValid).toBe(true);
      // RATE -> 2 vowels (A, E). Fail.
      expect(validateWord('RATE', constraints, dictionaryCheck).error).toBe('Minimum 3 voyelles');
    });
  });
  ...
  describe('Inversion Logic', () => {
    it('should swap imposed and forbidden letters', () => {
      ...
      expect(validateWord('RARE', constraints, dictionaryCheck).isValid).toBe(true);

      // "YETI" -> Has T, No A.
      // Inverted: Must have A, No T.
      // YETI fails Imposed A first.
      expect(validateWord('YETI', constraints, dictionaryCheck).error).toBe('Doit contenir A');
      
      // "TARTINE" -> Has T and A.
      // Inverted: Must have A (OK), No T (Fail).
      expect(validateWord('TARTINE', constraints, dictionaryCheck).error).toBe('Ne doit pas contenir T');
    });
  });
});
