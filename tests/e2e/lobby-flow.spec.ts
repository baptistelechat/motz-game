...
test.describe("Lobby Flow (Story 2.3)", () => {
  test.setTimeout(180000); // Increase timeout for slower environments
  ...
  test("[P1] Ready State & Game Start: Host and Joiner interaction", async ({
    ...
    console.log(
      `DEBUG: Direct request to /room/${code} returned status: ${response.status()}`,
    );
    ...
    try {
      ...
      try {
        await expect(pageB.getByTitle("Waiting").first()).toBeVisible({
          timeout: 10000,
        });
      } catch (e) {
        console.log(
          "DEBUG: Realtime update timed out or failed. Reloading to verify DB state...",
          e,
        );
        await pageB.reload();
        await expect(pageB.getByTitle("Waiting").first()).toBeVisible({
      ...
      await expect(page.getByRole("button", { name: "Lancer" })).toBeDisabled();
      await expect(
        page.getByText("Tous les joueurs doivent être prêts"),
      ).toBeVisible();
