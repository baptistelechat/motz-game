...
- [ ] Database Schema & RLS
  - [ ] Créer l'enum `game_status` ('LOBBY', 'PLAYING', 'FINISHED').
  - [ ] Créer la table `public.games`.
  ...
  - [ ] Policies:
    ...
    - `SELECT`: Public (Permettre de rejoindre si on a le code). *Note: On pourrait restreindre, mais pour l'instant Public simplifie le "Rejoindre".*
    - `UPDATE`: Host only (`auth.uid() = host_id`).

- [ ] Server Action & Logic
  - [ ] Installer `nanoid` (et `react-qr-code`).
  - [ ] Créer `src/lib/utils/game-code.ts` : Fonction de génération de code (6 chars, alphanum, uppercase).
  - [ ] Créer `src/app/actions/game-actions.ts` :
    - `createGame()` : Génère code, Insère en DB, Retourne le code.
    - Gestion d'erreur (Collision de code -> Retry).

- [ ] UI Implementation
  - [ ] Modifier `src/app/page.tsx` : Ajouter bouton "Créer une partie" (Call `createGame`).
  - [ ] Créer la page `src/app/room/[code]/page.tsx`.
    - Server Component : Fetch game details by code.
    - 404 si code invalide.
  - [ ] Créer `src/components/game/lobby-header.tsx` : Affiche le Code et le QR Code.
  - [ ] Intégrer `react-qr-code`.
...
### References
- [Architecture: Data Architecture](file:///c:\Users\DM\Desktop\DEV\perso\motz-game\_bmad-output\planning-artifacts\architecture.md#L131)
- [Story 2.1 Source](file:///c:\Users\DM\Desktop\DEV\perso\motz-game\_bmad-output\planning-artifacts\epics.md#L200)
